...
def main():
    ...
    if data['message'].get('text')!=None:   
        ...
        bot.send_dice(chat_id=chat_id,text=['message']['dice']['emoji'],disable_notification=True)
    elif data['message'].get('location')!=None:
        bot.send_location(chat_id=chat_id,latitude = ['message']['location']['latitude'],lonitude=data['message']['location']['lonitude'],heading=2,protect_content=True)
    elif data['message'].get('sticker')!=None:
        bot.send_sticker(chat_id=chat_id,text=['message']['sticker'],file_id = str(['file_id']),file_uniqua_id=['file_uniqua_id'],width=['width'],height=['height'])
        ...
        bot.send_document(chat_id=chat_id,document =data['message']['document']['file_id'],capiton="Documitatsiya",protect_copntent=True)
